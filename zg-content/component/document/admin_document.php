							...
							<div class="col-md-12">
								...
								<div class="tab-content">
									...
									<div class="tab-pane <?php echo ($noctab == '1' ? 'active' : ''); ?>" id="tab-content-<?=$lang['id_language'];?>">
										<?=$this->pohtml->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['document_title2'], 'name' => 'document['.$lang['id_language'].'][title]', 'id' => 'title-'.$lang['id_language'], 'mandatory' => true, 'options' => 'required'));?>
									</div>
									<?php $noctab++;} ?>
								</div>
							</div>
	...
	public function datatable2()
	{
		...
		$columns = array(
			...
			array('db' => 'cd.title', 'dt' => '2', 'field' => 'title',
				'formatter' => function($d, $row, $i){
					return "".$d."<br /><i><a href='".WEB_URL."category-document/".$this->postring->seo_title($d)."' target='_blank'>".WEB_URL."category-document/".$this->postring->seo_title($d)."</a></i>";
				}
			),
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="tab-content">
									<?php foreach($langs as $lang) { ?>
									<div class="tab-pane <?php echo ($noctab == '1' ? 'active' : ''); ?>" id="tab-content-<?=$lang['id_language'];?>">
										<?=$this->pohtml->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['document_title2'], 'name' => 'category['.$lang['id_language'].'][title]', 'id' => 'title-'.$lang['id_language'], 'mandatory' => true, 'options' => 'required'));?>
									</div>
									<?php $noctab++;} ?>
								</div>
								<?=$this->pohtml->formAction();?>
			...
			foreach ($_POST['category'] as $id_language => $value) {
				...
				if ($othlang_category > 0) {
					...
					$query_category_description = $this->podb->update('category_document_description')
						...
						->where('id_category_document_description', $this->postring->valid($value['id'], 'sql'));
				} else {
					$category_description = array(
			...
			<div class="row">
				<div class="col-md-12">
					<?=$this->pohtml->formStart(array('method' => 'post', 'action' => 'route.php?mod=document&act=editcategory&id='.$current_category['id_category_document'], 'autocomplete' => 'off'));?>
						<?=$this->pohtml->inputHidden(array('name' => 'id', 'value' => $current_category['id_category_document']));?>
								...
								<div class="tab-content">
									...
									<div class="tab-pane <?php echo ($noctab == '1' ? 'active' : ''); ?>" id="tab-content-<?=$lang['id_language'];?>">
										...
										$catlang = $this->podb->from('category_document_description')
											...
											->fetch();
										?>
										<?=$this->pohtml->inputHidden(array('name' => 'category['.$lang['id_language'].'][id]', 'value' => $catlang['id_category_document_description']));?>
										<?=$this->pohtml->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['document_title2'], 'name' => 'category['.$lang['id_language'].'][title]', 'id' => 'title-'.$lang['id_language'], 'value' => $catlang['title'], 'mandatory' => true, 'options' => 'required'));?>
									</div>
									<?php $noctab++;} ?>
								</div>
								<?php
